...
def run_style_transfer(
    ...
    while n_iter[0] <= max_iter:
        ...
        def closure():
            ...
            layer_losses = [
                weights[i] * loss_fns[i](out[i], targets[i])
    ...
    # Compute HR targets
    style_targets_hr = [
        GramMatrix()(A).detach() for A in vgg(style_var_hr, style_layers)
    ]
    ...
    while n_iter_hr[0] <= max_iter_hr:
        ...
        def closure_hr():
            ...
            layer_losses_hr = [
                weights[i] * loss_fns[i](out_hr[i], targets_hr[i])
                for i in range(len(loss_layers))
            ]
...
def run_style_transfer_single(content_img, style_img, resolution="low", max_iter=300, style_weight=1e10, content_weight=1e5):
    ...
    return out_img_hr if resolution == "high" else out_img_lr
